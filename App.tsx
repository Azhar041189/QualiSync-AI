...
const DEFAULT_AGENTS: AgentProduct[] = [
  { id: 'sec_01', name: 'Zero-Trust Security Auditor', description: 'Auto-injects SQLi and XSS payloads into input fields.', icon: '', category: 'Security', price: '$499/mo', installed: true, role: AgentRole.SECURITY },
  { id: 'perf_01', name: 'Lighthouse Performance Guard', description: 'Runs Lighthouse CI checks on every build.', icon: '', category: 'Performance', price: '$299/mo', installed: true, role: AgentRole.PERFORMANCE },
  ...
  { id: 'data_01', name: 'Synthetic Data Fabricator', description: 'Generates GDPR-compliant PII mock data.', icon: '', category: 'Data', price: '$199/mo', installed: true, role: AgentRole.IDLE }
];
...
const App: React.FC = () => {
  ...
  const startSimulation = async () => {
    ...
    try {
      ...
      const dataAgent = marketplaceAgents.find(a => a.id === 'data_01' && a.installed);
      if (dataAgent) {
        setActiveRole(AgentRole.IDLE);
        addLog(AgentRole.IDLE, "Generating Synthetic Data...", 'info');
        await new Promise(r => setTimeout(r, 800)); // Simulate work
        addLog(AgentRole.IDLE, "Data Generated: 50 Records", 'success', "Mock users created: { id, name, email, ... }");
      }
  ...
  const renderSidebar = () => (
    <div className={`w-20 border-r flex flex-col items-center py-6 gap-4 z-20 overflow-y-auto scrollbar-hide shadow-lg transition-colors duration-300 ${theme === 'dark' ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'
      }`}>
      ...
      <div className="mt-auto pt-4 w-full flex justify-center">
        <button
          onClick={toggleTheme}
          className={`p-3 rounded-xl transition-all ${theme === 'dark' ? 'text-yellow-400 hover:bg-slate-800' : 'text-slate-400 hover:bg-slate-100'
            }`}
        >
  ...
  return (
    <div className={`flex h-screen w-screen font-sans selection:bg-blue-500/30 transition-colors duration-300 ${theme === 'dark' ? 'bg-slate-900 text-slate-100' : 'bg-[#f8fafc] text-slate-900'
      }`}>
      {renderSidebar()}
      ...
      <main className="flex-grow flex flex-col overflow-hidden relative">
        ...
        <div className="absolute top-0 right-0 p-4 z-10 pointer-events-none">
          <div className={`flex items-center gap-2 backdrop-blur-md border px-4 py-2 rounded-full text-xs font-medium shadow-sm ${theme === 'dark' ? 'bg-slate-800/80 border-slate-700 text-slate-300' : 'bg-white/80 border-slate-200 text-slate-600'
            }`}>
          ...
          {activeTab === 'agents' && (
            <div className="flex h-full animate-fade-in">
              <div className={`w-1/3 border-r p-6 flex flex-col z-10 shadow-sm ${theme === 'dark' ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>
                ...
                <textarea
                  className={`flex-grow border rounded-xl p-4 mb-4 text-sm resize-none focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all ${theme === 'dark' ? 'bg-slate-800 border-slate-700 text-slate-200 placeholder:text-slate-600' : 'bg-slate-50 border-slate-200 text-slate-900'
                    }`}
                  value={inputStory}
                      ...
                      <div key={log.id} className="animate-slide-in flex gap-3">
                        ...
                        <div className="flex-grow">
                          <div className="flex items-center gap-2 mb-1">
                            <span className={`font-bold px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider ${log.role === AgentRole.CRITIC ? 'bg-red-500/20 text-red-400' :
        ...
        <div className={`h-8 border-t flex items-center px-4 justify-between text-[10px] font-mono select-none ${theme === 'dark' ? 'bg-slate-900 border-slate-800 text-slate-500' : 'bg-white border-slate-200 text-slate-400'
          }`}>
...
const SidebarIcon = ({ icon, active, onClick, tooltip, theme }: any) => (
  <button onClick={onClick} className={`p-3 rounded-xl transition-all group relative duration-200 ${active
    ...
    {icon}
    <div className={`absolute left-16 top-1/2 -translate-y-1/2 text-xs px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 shadow-xl border pointer-events-none z-50 whitespace-nowrap transition-all duration-200 transform translate-x-[-10px] group-hover:translate-x-0 ${theme === 'dark' ? 'bg-slate-800 text-white border-slate-700' : 'bg-white text-slate-700 border-slate-200'
      }`}>
      {tooltip}
      {/* Little arrow */}
      <div className={`absolute top-1/2 -left-1 -translate-y-1/2 w-2 h-2 rotate-45 border-b border-l ${theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'
        }`}></div>
    </div>
  </button>
);
