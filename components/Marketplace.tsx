...
const Marketplace: React.FC<MarketplaceProps> = ({ agents, onToggle }) => {
  ...
  return (
    <div className="p-8 h-full overflow-y-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-500">Extend your QA Squad with specialized Digital Workers. Installed agents will automatically participate in the "Agent Lab" workflow.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {agents.map((agent) => (
          <div key={agent.id} className={`bg-white border rounded-xl p-6 transition-all group flex flex-col h-full shadow-sm ${
             agent.installed ? 'border-blue-500 ring-1 ring-blue-500 shadow-md' : 'border-slate-200 hover:border-blue-300'
          }`}>
            <div className="flex justify-between items-start mb-4">
              <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 group-hover:border-slate-300 transition-colors text-2xl">
                {agent.category === 'Security' && <Shield className="text-red-500" />}
                {agent.category === 'Performance' && <Zap className="text-yellow-500" />}
                {agent.category === 'Accessibility' && <Eye className="text-blue-500" />}
                {agent.category === 'Data' && <Database className="text-green-500" />}
              </div>
